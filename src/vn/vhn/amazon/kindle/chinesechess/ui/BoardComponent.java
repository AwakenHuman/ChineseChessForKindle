    ...
    implements KeyListener
{
    ...
    {
        gameTimer.stop();   // stop the game time
        try
        {
            if(KOptionPane.showConfirmDialog(this,
                ...
                if(Main.posmode==1){  // check mate but continue
    ...
    {
        if(pos.isMate())  // is it check mate situation ?
        {
            message = response >= 0 ? Main.rLang.getString("youLost") : Main.rLang.getString("youWon");
            int i;
            if(Main.posmode != 0 && response < 0){  // remember you solve it
                Main.rsData[0] = 0;
...
//                    .concat("  ".concat(String.valueOf(mvLast))), 50, 250);
    ...
    {
        // PHO: should check outside the chessboard
        // PHO: currently has a bug that you can choose a square 
        //      outside the right edge of chessboard (cursorX = 9)
        if (cursorX <= 8)
            clickSquare(Position.COORD_XY(cursorX + 3, cursorY + 3));
        //debug(String.valueOf(cursorX).concat(String.valueOf(cursorY)));
    }
    ...
    private void clickSquare(int sq)
    {
        if(sq<0 || sq>255){  
            ...
            if(sqSelected > 0) {
                ...
                {
                    sqSelected = 0;
                    // PHO: should give reasons of invalid move
                    
                    if (pos.checked())  // PHO: remind player to save the king first if checked mate
                        infoLabel.setText(Main.rLang.getString("saveKing"));                        
                    else
